	...
	public static User login(String id, String password) {
		...
		try {
			...
			String query = "SELECT user.id, user.lastName, user.firstName, user.email, user.password,"
					+ "address.id, address.no, address.appt, address.street, address.zip, address.city, address.state, address.country,user.role"
			...
			if(rs.next()) {
				...
				user = new User(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),rs.getString(14), address);
			}	
